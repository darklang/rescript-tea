        ...
        list{
          input'(
            list{
              ...
              onClick'(~key=key ++ string_of_bool(todo.completed), Check(todo.id, !todo.completed)),
            },
            list{},
          ),
          label(
...
let viewEntries = (visibility, entries) => {
  ...
  section(
    ...
    list{
      ...
      ul(list{Html.Attributes.class("todo-list")}, List.map(viewEntry, List.filter(isVisible, entries))),
    },
  )
}
